...
def fmha_kernel(Q, K, V, Out,
    ...
    # We are launching a 1D grid of size NUM_VSM.
    # Each VSM handles a subset of all Q tiles across all batches and heads in a round-robin fashion.
    ...
    num_tiles_q = ct.cdiv(SEQ_LEN_Q, TILE_M)
    
    # Total tiles = Batch * Heads * num_tiles_q
    # We can get Batch from Q.shape[0] if it's available as a constant or runtime value.
    # Since Q is passed in, Q.shape[0] is dynamic.
    batch_dim = Q.shape[0]
    total_tiles = batch_dim * H * num_tiles_q
    ...
    for global_tile_idx in range(bid_x, total_tiles, NUM_VSM):
        # Calculate batch, head, and Q-tile index from global_tile_idx
        # global_tile_idx = (batch_idx * H + head_idx) * num_tiles_q + tile_idx
        # But wait, flattening:
        ...
        offs_m = tile_idx * TILE_M + ct.arange(TILE_M, dtype=np.int32)  # [TILE_M]
        offs_m += input_pos
        ...
        q = ct.load(
            Q, index=(batch_idx, head_idx, tile_idx, 0), shape=(1, 1, TILE_M, TILE_D)
        ).reshape((TILE_M, TILE_D))  # [TILE_M, TILE_D]

        # loop over k, v and update accumulator
        m_end = input_pos + (tile_idx + 1) * TILE_M
        k_seqlen = K.shape[2]
        if CAUSAL:
            # when kv pos could exceed q pos
            mask_start = (input_pos + tile_idx * TILE_M) // TILE_N
        ...
        ct.store(Out, index=(batch_idx, head_idx, tile_idx, 0), tile=acc)
