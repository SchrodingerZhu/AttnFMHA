...
def _attn_fwd(
    ...
    """
    Triton kernel for Fused Multi-Head Attention (FMHA).
    Flattened single-function structure matching CUTILE default implementation.
    """
...
def triton_fmha(Q, K, V, qk_scale=None, input_pos=0, tile_m=128, tile_n=64, query_group_size=1, causal=False):
    """
    Triton FMHA with flattened single-function structure.
    """
    BLOCK_M = tile_m
    BLOCK_N = tile_n
    ...
    grid = (triton.cdiv(N_CTX, BLOCK_M), BATCH * N_HEADS, 1)
